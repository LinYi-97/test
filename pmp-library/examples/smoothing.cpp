...
void Viewer::process_imgui()
{
    ...
    if (ImGui::CollapsingHeader("Smoothing", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        if (ImGui::Button("Explicit Smoothing"))
        {
           ...
           auto fp = mesh_.add_face_property<Normal>("f:normal");
            for (auto f : mesh_.faces())
            {
                fp[f] = face_normal(mesh_, f);
            }
            write(mesh_, "E:\\pmp-data\\bunny.stl");
            update_mesh();
        }
...
int main(int argc, char** argv)
{
    ...
    if (argc)
        window.load_mesh("E:\\pmp-library\\data\\off\\bunny.off");
    return window.run();
#else
    Viewer window("Smoothing", 800, 600);
